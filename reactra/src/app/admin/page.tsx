...
// ---------------- Teams: mark round 1 qualification, delete team ----------------
function TeamsTab() {
  ...
  // Cascade delete: removes the team plus every doc in other collections that
  // points back at it (scores, seating, definition assignment). This works
  // regardless of whether the student who created the team still has a user
  // account — unlike deleting via the Users tab, this doesn't depend on
  // finding an owning user first, so it's the reliable way to fully remove a
  // team, including cleaning up after someone was deleted directly in the
  // Firebase Console instead of through this app.
  async function handleDeleteTeam(team: Team) {
    setError("");
    const ok = window.confirm(
      `Delete "${team.teamName}"? This also removes its scores, seating assignment, ` +
        `and frees up its assigned definition (if any). This can't be undone.`
    );
    if (!ok) return;
